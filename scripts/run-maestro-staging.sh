...
EXTRA_ARGS=("${@:2}")
# npm run test:e2e:checkout -- fluxo.yaml passa a pasta e o yaml; Maestro rodaria os dois.
if [[ ${#EXTRA_ARGS[@]} -gt 0 && "${EXTRA_ARGS[0]}" == *.yaml ]]; then
  TARGET="${EXTRA_ARGS[0]}"
...
: "${E2E_CHECKOUT_BILLING_PERIOD:=MONTHLY}"
: "${E2E_CARD_CVV:=123}"
...
export E2E_CHECKOUT_PRODUCT_ID E2E_CHECKOUT_PROTOCOL_PRODUCT_ID E2E_CHECKOUT_BILLING_PERIOD E2E_CARD_CVV
...
MAESTRO_ENV_ARGS=(
  ...
  -e "E2E_CHECKOUT_BILLING_PERIOD=${E2E_CHECKOUT_BILLING_PERIOD}"
